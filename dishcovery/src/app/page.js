...
export default function Homepage() {
  ...
  const filteredFood = FOOD_DATA.filter(food => {
    ...
    const matchCuisine = selectedCuisine.length === 0 || selectedCuisine.includes(food.category)
    const matchPrice = priceRange.length === 0 || (parseFloat(food.price) >= priceRange[0] && parseFloat(food.price) <= priceRange[1])
      ...
      <header className="flex items-center justify-between px-6 py-4 border-b sticky top-0 bg-white z-10">
        <div className="flex items-center gap-8">
          ...
          <nav className="hidden md:flex gap-6 text-sm font-medium text-gray-600">
            ...
            <Link href="/random" className="text-orange-500">
              Random
            </Link>
          </nav>
        </div>
          ...
          <div className="space-y-6">
            <div>
              ...
              <div className="relative">
                ...
                <Input placeholder="Search dishes..." className="pl-8" value={search} onChange={(e) => setSearch(e.target.value)} />
              </div>
            </div>
              ...
              {["Japanese", "American", "Italian", "Mexican", "Thai"].map(
                (item) => (
                  <div key={item} className="flex items-center gap-2 mb-2">
                    <Checkbox id={item} checked={selectedCuisine.includes(item)} onCheckedChange={(checked) => {
                      if (checked) {
                        setSelectedCuisine([...selectedCuisine, item])
                      } else {
                        setSelectedCuisine(selectedCuisine.filter(c => c !== item))
                      }
                    }} />
            ...
            <div>
              ...
              <Slider defaultValue={[50]} max={100} step={1} onValueChange={(value) => setPriceRange([0, value])} />
